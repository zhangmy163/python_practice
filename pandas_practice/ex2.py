...
def changeItem(df1,df2,source,target):
    ...
    for item in dfj[source]:
        ...
        # print(df1.shape,"=======1")
        data1 = sorted((df1.where(df1[source] == item).dropna())[target].values)
        data2 = sorted((df2.where(df2[source] == item).dropna())[target].values)
        # print(df1.shape,"=======2")
...
if __name__ == "__main__":
    ...
    # print("===================车型对比=================")
    # f1 = str(base_dir) + "/file/old/cal/meta-car-model-filtered.csv"
    # df1 = initDfCsv(f1,[]).drop_duplicates()
    # # print(df1.index)
    # f2 = str(base_dir) + "/file/new/cal/meta-car-model-filtered.csv"
